...
# Get a handle to the shared library. Search order:
#   1. Directory of this lib.py file (openblt package folder)
#   2. Current working directory
#   3. System library path (LD_LIBRARY_PATH / PATH)
_pkg_dir = os.path.dirname(os.path.abspath(__file__))

# Python 3.8+ on Windows no longer searches the DLL's own directory for its
# dependencies (security change). Explicitly add the package folder so that
# peer DLLs (e.g. peak_pcanusb.dll) placed alongside libopenblt.dll are found.
if sys.platform.startswith('win') and hasattr(os, 'add_dll_directory'):
    os.add_dll_directory(_pkg_dir)

if os.path.exists(os.path.join(_pkg_dir, sharedLibrary)):
    sharedLibraryHandle = ctypes.CDLL(os.path.join(_pkg_dir, sharedLibrary))
elif os.path.exists(os.path.join(os.getcwd(), sharedLibrary)):
